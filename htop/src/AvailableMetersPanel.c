...
AvailableMetersPanel* AvailableMetersPanel_new(Settings* settings, Header* header, Panel* leftMeters, Panel* rightMeters, ScreenManager* scr, ProcessList* pl) {
   ...
   for (i = 2; Platform_meterTypes[i]; i++) {
   ...
   if (cpus > 1) {
      ...
      Panel_add(super, (Object*) ListItem_new("CoreTemp", 1 | (30 << 16)));
   }
   ...
   if (cpus > 1) {
      ...
      Panel_add(super, (Object*) ListItem_new("CoreFreq", 1 | (31 << 16)));
   }
   ...
   if (fs > 1) {
      for (i = 1; i <= fs; i++) {
         ...
         Panel_add(super, (Object*) ListItem_new(buffer, i | (32 << 16)));
      }
   } else {
      Panel_add(super, (Object*) ListItem_new("fs", 1 | (32 << 16)));
   }
   ...
   // Handle (&CPUTXTMeter_class)
   type = &CPUXMeter_class;
   cpus = pl->cpuCount;
   if (cpus > 1) {
      for (i = 1; i <= cpus; i++) {
         char buffer[50];
         xSnprintf(buffer, 50, "%s %d", type->uiName, i);
         Panel_add(super, (Object*) ListItem_new(buffer, i | 1 << 16));
      }
   }
   return this;
}
